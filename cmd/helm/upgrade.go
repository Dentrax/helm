...
func newUpgradeCmd(cfg *action.Configuration, out io.Writer) *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if client.Install {
				...
				if _, err := histClient.Run(args[0]); err == driver.ErrReleaseNotFound {
					...
					instClient.WaitIgnoreUnschedulable = client.WaitIgnoreUnschedulable
	...
	f.BoolVar(&client.WaitIgnoreUnschedulable, "wait-ignore-unschedulable", false, "if set and --wait enabled, wait will only respect the Nodes that are schedulable and will ignore Pods with NotReady conditions to be scheduled on unschedulable Nodes")
